...
use crate::{
    constants::{
        gas_constants::{
            NO_DEPOSIT, TGAS_FOR_GET_ACCOUNT_STAKED_BALANCE,
            TGAS_FOR_IS_WHITELISTED, TGAS_FOR_PING, TGAS_FOR_SELECT_POOL_AFTER_CHECK_WHITELIST,
            TGAS_FOR_UNSTAKE, TGAS_FOR_WITHDRAW, TGAS_FOR_DEPOSIT_AND_STAKE,
        },
...
impl StakerAction for RestakingBaseContract {
    ...
    fn withdraw_all(&mut self, account_id: AccountId, pool_id: PoolId) -> PromiseOrValue<U128> {
        ...
        ext_staking_pool::ext(account_id.clone())
            ...
            .then(
                Self::ext(env::current_account_id())
                    .withdraw_callback(account_id, withdrawable_pending_withdrawals.iter().map(|e|e.withdrawal_certificate).collect_vec()),
            ).into()
    }
}
...
impl RestakingBaseContract {
    pub(crate) fn internal_create_pending_withdrawal_for_staker(
        ...
        storage_manager.execute_in_storage_monitoring(|| {
            self.internal_use_account(&staker.staker_id, |account| {
                account.pending_withdrawals.insert(&pending_withdrawal.withdrawal_certificate, &pending_withdrawal);
    ...
    pub(crate) fn internal_get_staker_selected_pool_or_panic(&self, account_id: &AccountId) -> PoolId {
        self.stakers
            .get(account_id)
