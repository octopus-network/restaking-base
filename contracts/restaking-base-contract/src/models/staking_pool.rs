...
impl RestakingBaseContract {
    ...
    pub(crate) fn internal_save_staking_pool(&mut self, pool_id: &PoolId, staking_pool: &StakingPool) {
        self.staking_pools.insert(pool_id, &staking_pool);
    }

    pub(crate) fn internal_use_staking_pool_or_panic<F, R>(&mut self, pool_id: &PoolId, mut f: F) -> R 
    where
    ...
    pub(crate) fn internal_get_staking_pool_by_staker_or_panic(&self, staker_id: &StakerId) -> StakingPool {
