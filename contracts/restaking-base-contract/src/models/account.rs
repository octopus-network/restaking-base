...
pub struct Account {
    ...
    pub pending_withdrawals: UnorderedMap<WithdrawalReceiptId, PendingWithdrawal>,
}
...
impl Account {
    ...
    pub fn save_legacy_shares(&mut self, shares: ShareBalance, pool_id: PoolId) {
        let new_shares = shares.checked_add(self.legacy_shares.get(&pool_id).unwrap_or(&0).to_owned()).unwrap();
